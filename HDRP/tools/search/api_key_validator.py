...
def validate_bing_api_key(
    ...
    """Validate a Bing Search API key.
    
    Args:
        api_key: The API key to validate. If None, checks BING_API_KEY env var.
        raise_on_invalid: If True, raises APIKeyError on validation failure.
                         If False, returns (False, error_message) instead.
    ...
    """
    # Get the effective API key
    effective_key = api_key or os.getenv("BING_API_KEY")
    
    # Check if key is missing
    if not effective_key:
        error_msg = _get_bing_missing_key_error()
        if raise_on_invalid:
            raise APIKeyError(error_msg)
        return False, error_msg
    ...
    # Check if key is just whitespace
    if not effective_key.strip():
        error_msg = _get_bing_empty_key_error()
        if raise_on_invalid:
            raise APIKeyError(error_msg)
        return False, error_msg
    ...
    if effective_key.lower() in placeholder_values:
        error_msg = _get_bing_placeholder_key_error(effective_key)
        if raise_on_invalid:
            raise APIKeyError(error_msg)
        return False, error_msg
    
    # Basic format validation (Bing keys are typically 32-character hex strings)
    if len(effective_key) < 16:
        error_msg = _get_bing_invalid_format_error(effective_key)
        if raise_on_invalid:
            raise APIKeyError(error_msg)
        return False, error_msg
...
def _get_bing_missing_key_error() -> str:
    """Generate error message for missing Bing API key."""
    return """Bing API key not configured.

To use Bing Web Search, you need to set your API key:

  1. Create a Bing Search resource in Azure Portal:
     https://portal.azure.com/#create/Microsoft.CognitiveServicesBingSearch-v7
  
  2. Get your API key from the resource's "Keys and Endpoint" section.
  
  3. Set the environment variable:
     
     export BING_API_KEY="your-actual-subscription-key"
  
  4. Or pass it explicitly via CLI:
     
     --api-key "your-actual-subscription-key"
...
def _get_bing_empty_key_error() -> str:
    """Generate error message for empty Bing API key."""
    return """Bing API key is empty.

The BING_API_KEY environment variable is set but contains only whitespace.

To fix this:

  1. Get a valid API key from Azure Portal:
...
def _get_bing_placeholder_key_error(placeholder: str) -> str:
    """Generate error message for placeholder Bing API key."""
    return f"""Bing API key appears to be a placeholder: "{placeholder}"

You need to replace this with your actual API key from Azure.

To fix this:

  1. Get your actual API key from Azure Portal:
     https://portal.azure.com (Cognitive Services - Bing Search)
...
def _get_bing_invalid_format_error(key: str) -> str:
    """Generate error message for invalid Bing API key format."""
    masked_key = key[:4] + "..." if len(key) > 4 else "***"
    
    return f"""Bing API key appears to be invalid: "{masked_key}"

The provided API key is too short or has an unexpected format.
Bing API keys are typically 32-character hexadecimal strings.

To fix this:

  1. Verify you have the correct API key from Azure Portal:
     https://portal.azure.com (Cognitive Services - Bing Search)
